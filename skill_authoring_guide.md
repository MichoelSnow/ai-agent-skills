...
The workflow imposed by a skill should scale with the scope, risk, and uncertainty of the task.

- Do not require a full planning workflow for trivial or well-bounded changes.
- Do not skip clarification or planning when uncertainty could materially change the solution.
- Where a skill covers tasks of substantially different sizes, define a lightweight path and an expanded path.
- Prefer the minimum process necessary to perform the task reliably.
...
Skills must distinguish reasoning about work from authorization to perform it.

Discussion, clarification, planning, diagnosis, review, or acknowledgement of a problem does not by itself authorize modification of files or other state.

When authorization to mutate state is ambiguous, prefer not to mutate.

A skill that intentionally transitions from analysis to modification should make that transition explicit.
...
1. Capture intent and examples.
 - When a skill is intended to correct recurring agent behavior, include representative real-world failure cases among its evaluation prompts.
...
- `agent-skill-best-practices` from `nyosegawa/skills`: https://github.com/nyosegawa/skills/blob/main/agent-skill-best-practices.md
